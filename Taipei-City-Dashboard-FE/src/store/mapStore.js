...
export const useMapStore = defineStore("map", {
    ...
    actions: {
                ...
                onAdd: function () {
                    ...
                    for (let route of allRoutes) {
                        ...
                        tb.loadObj(options, function (model) {
                            ...
                            car.addEventListener('ObjectChanged', self.onIsPlayingChanged, false)
                            tb.add(car)
                            car.followPath(
                                lineOptions,
                                function () {
        ...
        async onIsPlayingChanged(e)
        {
            const getDistanceByLatLng=(curLat, curLng, placeLat, placeLng)=> {
                const deg2rad = (deg) => {
            ...
            let self=this;
            let lan = e.detail.action.position[0]
            let log = e.detail.action.position[1]

            let rd = await this.getdata();

            // console.log(lan)
            // console.log(rd.data.features)
            let points = rd.data.features.map(x=>
                getDistanceByLatLng(x.geometry.coordinates[0],x.geometry.coordinates[1],
                    lan , log
                ) );
            //console.log(points)

            let indexObj = points.find(x=> x < 0.06 );

            if (indexObj ==null)
                return ;
            let index = points.findIndex(x=> x < 0.06 );
            //console.log(indexObj)

            console.log(index)
            let monument = [lan,log];
        ...
        <h2> 7-11點位 : 5 </h2>
            ...
            let popup = new mapboxGl.Popup({ closeOnClick: false , offset: 25 })
                .setLngLat(monument)
                .setHTML(textHtml)
                .addTo(self.map);
        ...
        {
            let rs = await axios
                .get(`/mapData/test_route270Point.geojson`);
